...
parser = argparse.ArgumentParser(description="A powerful and advanced admin panel buster tool in python. ")
parser.add_argument("-u", "--url", help=" Target URL to check for admin panel. ", dest="target")
parser.add_argument("-e", "--ext", help=" To check against specific extension only. ", dest="ext")
...
parser.add_argument("-vl", "--verylarge", help=" To check against very large wordlist (Not Recommended). ", action="store_true", default=False, dest="verylarge")
...
ext = args.ext
if ext:
    ext_list =['php','asp','html']
    if ext not in ext_list:
        print(Back.RED + "[-] Invalid extension.")
        print(Fore.YELLOW + "EXITING NOW...")
        quit()
    else:
        new_ext_list = [item for item in ex_list if item!=ext]
    try:
    ...
    for item in urls_list:
        ...
        try:
            ...
            if req == 200:
                ...
                print(Back.GREEN + "[+] Redirection on url: ", url)
                output.append(url)
            else:
                print(Fore.RED + "[-] ",url)
        except:
